...
func Update(_delta:float):	
	if Input.is_action_just_pressed("ui_attack") and bob.attack_left != 1: # if left click is pressed and we still have attack left
...
func next_animation_selector_attacking():
	...
	if not bob.AIMING_MOUSE:
		if bob.last_dir_attack_array[0].x != 0: # if the player was moving towards left or right
			...
			if bob.last_dir_attack_array[0].x > 0:
			...
			elif bob.last_dir_attack_array[0].x < 0:
				animated_sprite_2d.flip_h = true # facing right
				animation_player.play("atk_left_1_tempo")
		
		elif bob.last_dir_attack_array[0].y > 0: # if the player was moving towards bottom
			animated_sprite_2d.flip_h = false # facing right
			animated_sprite_2d.play(bob.attacks_array[1][0])
			animation_player.play("atk_down_1_tempo")
		
		elif bob.last_dir_attack_array[0].y < 0: # if the player was moving towards top
			animated_sprite_2d.flip_h = false # facing right
